 ...
 * @author USER
 */
public class TugasNo2_Queue {
    int max, size, front, rear;
    TugasNo2_Mahasiswa[] Q;

    public TugasNo2_Queue(int n) {
        max = n;
        Create();
    }

    public void Create() {
        Q = new TugasNo2_Mahasiswa[max];
        size = 0;
        front = rear = -1;
    }
    ...
    public void Enqueue(TugasNo2_Mahasiswa data) {
        if (IsFull()) {
    ...
    public TugasNo2_Mahasiswa Dequeue() {
        TugasNo2_Mahasiswa data = new TugasNo2_Mahasiswa("", "", 0, 0);
        if (IsEmpty()) {
            System.out.println("Queue masih kosong");
        } else {
            data = Q[front];
